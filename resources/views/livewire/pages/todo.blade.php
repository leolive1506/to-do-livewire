<div>
    ...
    <x-button wire:click="toggle">Nova</x-button>

    <x-container class="mt-4">
        <div>
            <x-input name="search" placeholder="Digite para buscar..." class="w-full" />
        </div>
        @if ($isOpen)
            aberto
        @endif
            ...
            {{-- @forelse ($todos as $item)
                <div class="flex justify-between items-center">
                    <div class="flex flex-col gap-2">
                        <form action="{{ route('tarefas.checkbox', $item->id) }}"
                            id="form_completed_{{ $item->id }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="block mt-4">
                                <label for="completed_{{ $item->id }}" class="inline-flex items-center"
                                    onclick="document.querySelector('#form_completed_{{ $item->id }}').submit()">
                                    <input id="completed_{{ $item->id }}" type="checkbox"
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                        name="completed" {{ $item->completed === 1 ? ' checked' : '' }}>
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('tarefas.show', $item->id) }}">
                            <x-icons.eye />
                        </a>
                        <a href="{{ route('tarefas.edit', $item->id) }}">
                            <x-icons.pencil />
                        </a>
                        <form method="POST" action="{{ route('tarefas.destroy', $item->id) }}"
                            id="tarefa-delete-{{ $item->id }}">
                            @csrf
                            @method('DELETE')
                        </form>
                        <button
